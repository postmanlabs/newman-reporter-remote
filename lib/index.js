const os = require('os'),
    ...
    remoteReporter = function (newman, options, cr) {
        ...
        let collection = {
            ...
            // @note Do not set environment if `cr.environment` is undefined.
            // This check is required for non-newman agents.
            ...
            run = {
                id: runId,
                type: 'waterfall',
                source: options.source || `${PACKAGE_JSON.name}@${PACKAGE_JSON.version}`,
                agent: {
        ...
        newman.on('exception', function (err, data) {
            const exception = data.error instanceof SerialisedError ?
                data.error : new SerialisedError(data.error, true);

            addExecution(err,
                data,
                executions,
                EXECUTION_TYPES.exception,
                runId,
                exception.id,
                SELF_MODELS.exceptions);
