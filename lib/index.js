const os = require('os'),
    ...
    addExecution = function (err, data, executions, executionType, runId, selfId, selfModelName, startTime) {
        ...
        executions.push({
            ...
            cursor: data.cursor,
            args: data.cursor && {
                [EXECUTION_TYPES.request]: data.cursor.httpRequestId && {
                    $self: [SELF_MODELS.requests, data.cursor.httpRequestId]
                },
                event: data.cursor.scriptId, // events don't have a self-reference model
                [EXECUTION_TYPES.script]: data.cursor.execution && {
